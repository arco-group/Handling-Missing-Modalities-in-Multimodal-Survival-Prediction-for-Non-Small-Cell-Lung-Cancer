    ...
    try:
        for test_fold, val_fold, train_info, val_info, test_info, _ in get_cross_validation(path, "train", "val", "test", specific_fold=test_fold, print_fold=False):
            if "test" in sets:
                ...
                train_val_index.index = train_val_index.index.astype(int)
...
def provided_missing(data: np.array, labels: np.array, sets: List[str], path: str, test_fold: str = None, info_for_cv: dict = None, **kwargs) -> Tuple[int, np.array, np.array]:
    try:
        if 'test' in sets:
            data = data[info_for_cv['to_use'].astype(bool).values]
        for test_fold, val_fold, train_info, val_info, test_info, _ in get_cross_validation(path, "train", "val", "test", specific_fold=test_fold, print_fold=False):
            if "test" in sets:
                train_info = pd.concat([train_info, val_info], axis=0)

                train_info_filtered = train_info[train_info.iloc[:, 0].isin(pd.Series(data, name="ID").astype(int))]
                test_info_filtered = test_info[test_info.iloc[:, 0].isin(pd.Series(data, name="ID").astype(int))]
                yield test_fold, train_info_filtered.idx.values, test_info_filtered.idx.values
            else:

                # train_info, val_info = train_info.reset_index().set_index("ID"), val_info.reset_index().set_index("ID")
                train_val_index = pd.Series(data, name="ID").reset_index().set_index("ID")
                train_info_filtered = train_info[train_info.iloc[:, 0].isin(pd.Series(data, name="ID").astype(int))]
                val_info_filtered = val_info[val_info.iloc[:, 0].isin(pd.Series(data, name="ID").astype(int))]



                yield val_fold, train_val_index.loc[train_info_filtered.ID.astype(str), "index"].values, train_val_index.loc[val_info_filtered.ID.astype(str), "index"].values
    except FileNotFoundError as e:
        data = data[info_for_cv['to_use'].astype(bool).values]
        for test_fold, val_fold, train_info, test_info, _ in get_cross_validation(path, "train", "test", specific_fold=test_fold, print_fold=False):
...
def set_cross_validation(info_for_cv: pd.DataFrame, path: str, test_params: dict, val_params: dict, force: bool = False, **kwargs) -> None:
    ...
    options = {"holdout"  : holdout, "multilabelholdout": multilabelholdout, "kfold": kfold, "stratifiedkfold": stratifiedkfold, "multilabelstratifiedkfold": multilabelstratifiedkfold, "loo": leaveoneout,
               "bootstrap": bootstrap, "multilabelbootstrap": multilabelbootstrap, "predefined": predefined, "provided": provided, "provided_missing": provided_missing}
    ...
    test_cv = options[test_params["method"]](info_for_cv.ID.values, info_for_cv.target.values, sets=["train", "test"], info_for_cv=info_for_cv, **test_params)
    ...
    for fold, train_val_index, test_index in test_cv:
        ...
        val_cv = options[val_params["method"]](train_val_IDs.values, train_val_info.target.values, sets=["train", "val"], test_fold=f"{fold}_0", info_for_cv=info_for_cv, **val_params)
