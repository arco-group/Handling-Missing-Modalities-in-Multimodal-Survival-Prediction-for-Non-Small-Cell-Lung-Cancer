...
from CMC_utils.models import initialize_weights, load_pretrained_weights
...
def train_torch_model(model: torch.nn.Module, train_set: SupervisedTabularDatasetTorch, model_params: dict, model_path: str, val_set: SupervisedTabularDatasetTorch, train_params: Union[dict, DictConfig], test_fold: int = 0, val_fold: int = 0, use_weights: bool = True, **kwargs) -> None:
    ...
    """
    if use_weights:
        if train_set.label_type in ("single_risk_survival", "competing_risks_survival", "single_risk_survival_regression"):
        ...
        train_dataloader = DataLoader(train_set, batch_size=train_params.dl_params.batch_size, sampler=sampler, drop_last=False, worker_init_fn=seed_worker, num_workers=4, pin_memory=True, persistent_workers=True)
    else:
        train_dataloader = DataLoader(train_set, batch_size=train_params.dl_params.batch_size, shuffle=True, drop_last=False, worker_init_fn=seed_worker, num_workers=4, pin_memory=True, persistent_workers=True)
    val_dataloader = DataLoader(val_set, batch_size=train_params.dl_params.batch_size, shuffle=False, drop_last=False, num_workers=4, pin_memory=True, persistent_workers=True)
    ...
    if train_params.dl_params.load_pretrained_parts:
        model = load_pretrained_weights(model, pretrained_model_parts=kwargs.get(   'pretrained_model_parts', ''), fold=test_fold)
...
def model_predict(model, dataloader, criterions, regularizers, optimizer, tr_manager, train_params, phase: str = "val", **kwargs) -> Tuple[torch.nn.Module, float, pd.DataFrame]:
    ...
    with torch.set_grad_enabled(phase == 'train'):
        for *input_list, labels, idxs in pbar:
            ...
            if dataloader.dataset.label_type =="single_risk_survival_regression":
                preds = list(np.round(preds, dataloader.dataset.decimals))
                outputs = outputs.squeeze()
                labels = labels.astype(int).tolist()
                if isinstance(labels, int):
                    labels = [labels]
            elif dataloader.dataset.label_type =="continuous":
                preds = list(np.round(preds, dataloader.dataset.decimals))
            else:
                labels = labels.astype(int).tolist()
