...
__all__ = ["EarlyStopper", "ModelCheckpointer", "CustomScheduler", "TrainManager", "CustomLrSchedulerWithWarmup"]
...
class CustomLrSchedulerWithWarmup:
    """
    Scheduler to reduce the learning rate when the performance or the loss does not improve after certain epochs.
    """

    def __init__(self, learning_rates: dict, patience: int, verbose: int, min_epochs: int, model_parts: dict = None, epochs_no_improve: int = 0, learning_rate_idx: int = 0, idx_reverse_order: bool = False, invert_order_on_lr_end: bool = False, **kwargs):

        self.learning_rates, self.model_parts = self.create_learning_rates(model_parts=model_parts, min=learning_rates["min"], max=learning_rates["max"], steps=learning_rates["steps"], min_epochs=min_epochs)
        self.patience = patience
    ...
    @property
    def lr(self):
        lr_idx = max(min(self.state["learning_rate_idx"], len(self.learning_rates if self.model_parts is None else self.learning_rates[list(self.learning_rates.keys())[0]]) - 1), 0)
    ...
    def create_learning_rates(self, model_parts, **kwargs) -> list:
        """

        Create a list of learning rates from min to max with a given number of steps.
        Parameters
        ----------
        min: int
        max: int
        ...
        -------
        """
        if model_parts is None or model_parts.get('freeze_ms', False):
            return list(np.linspace(kwargs['min'] * 0.001, kwargs['max'], kwargs['min_epochs'])) + list(np.linspace(kwargs['max'], kwargs['min'], kwargs["steps"])), None
        else:
            learning_rates = dict()
            model_parts.pop('freeze_ms')
            for k, v in model_parts.items():
                learning_rates[k] = list(np.linspace(v['min'] * 0.001, v['max'], kwargs['min_epochs'])) + list(np.linspace(v['max'], v['min'], v["steps"]))
            return learning_rates, model_parts
            ...
            if self.next_step:
                if self.invert_order_on_lr_end:
                    if not self.state["idx_reverse_order"]:
                        ...
                        self.state["idx_reverse_order"] = False if self.state["learning_rate_idx"] < len(self.learning_rates if self.model_parts is None else self.learning_rates[list(self.learning_rates.keys())[0]]) - 1 else True
                    else:
                        self.state["learning_rate_idx"] -= 1
                ...
                elif self.state["learning_rate_idx"] < len(self.learning_rates if self.model_parts is None else self.learning_rates[list(self.learning_rates.keys())[0]]) - 1:
                    self.state["learning_rate_idx"] += 1
                    print_options[self.verbose](f"Learning rate changed to {self.lr}")
        else:
            self.state["learning_rate_idx"] += 1
            print_options[self.verbose](f"WARMUP: Learning rate changed to {self.lr}")
