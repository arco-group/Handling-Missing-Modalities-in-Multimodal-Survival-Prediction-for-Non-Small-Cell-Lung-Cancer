"""
Aggregate 5-fold test predictions from fold_specific experiments (Merlin CT embeddings).

For each experiment under outputs/fold_specific/{modality_combo}/{experiment}/,
navigates to:
  {provided_dir}/predictions/{pred_subdir}/0/0-0/
and concatenates {0,1,2,3,4}_0_test.csv into a single file:
  {provided_dir}/predictions/{pred_subdir}/aggregate_5fold_test.csv
...
BASE_DIR      = Path("/mimer/NOBACKUP/groups/naiss2023-6-336/AIDA_multimodal_F&C/outputs/fold_specific")
...
def find_fold_dir(pred_subdir: Path) -> Path | None:
    ...
    Three layouts are supported:
      1. pred_subdir/0/0-0/       (bimodal)
      2. pred_subdir/0/0-0-0/     (trimodal)
