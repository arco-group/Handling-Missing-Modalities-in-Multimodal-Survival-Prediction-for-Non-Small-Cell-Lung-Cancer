...
for task in "${tasks[@]}"; do
  for model in "${models[@]}"; do
    for target in "${targets[@]}"; do
      ...
      python ./main.py experiment="$experiment" continue_experiment=false experiment/preprocessing/label@preprocessing.label=cox experiment/databases@db=AIDA_${target} mode=tabular_cox-label_cix &
      pid=$!
      PIDS+=($pid)
      COUNT=$((COUNT + 1))
