...
for task in "${tasks[@]}"; do
  for model in "${models[@]}"; do
    for target in "${targets[@]}"; do
      ...
      python ./main.py experiment="$experiment" continue_experiment=false experiment/preprocessing/label@preprocessing.label=none experiment/databases@db="AIDA_${target}" mode=tabular_cix experiment/paths/system@_global_=alvis_cix experiment/model@model=${model,}_t &
      COUNT=$((COUNT + 1))
