...
for task in "${tasks[@]}"; do
  for model in "${models[@]}"; do
    for target in "${targets[@]}"; do
      ...
      python ./main.py experiment="$experiment" continue_experiment=true experiment/preprocessing/label@preprocessing.label="cox" mode=tabular_cox-label_cix experiment/databases@db=AIDA_${target} &
      pid=$!
      PIDS+=($pid)
      COUNT=$((COUNT + 1))
