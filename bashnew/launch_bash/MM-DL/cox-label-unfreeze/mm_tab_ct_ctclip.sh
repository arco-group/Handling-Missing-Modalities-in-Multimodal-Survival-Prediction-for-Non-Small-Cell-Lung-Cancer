...
for task in "${tasks[@]}"; do
  for model in "${models[@]}"; do
    for target in "${targets[@]}"; do
      experiment="AIDA_multimodal_CT+tabular_${task}_cox_label"
      ...
      mode=ct_tab_cox-label_cix \
      experiment/databases@dbs.0=CT_ctclip_AIDA_${target} \
      experiment/databases@dbs.1=AIDA_${target} \
      db_name=CT+tabular_multimodal_${target} &
      COUNT=$((COUNT + 1))
