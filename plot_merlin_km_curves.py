"""
Kaplan-Meier stratification plots for Merlin aggregated predictions.

Scans outputs/fold_specific/ for every Concat*+*_ multimodal experiment
across all modality combination directories (excluding 'late' and 'copy').

For each experiment produces two separate figures:
  {mod}_{arch}_{freeze}_adaptive.png — optimal log-rank cutoff (10th–90th percentile)
  {mod}_{arch}_{freeze}_median.png   — median score cutoff
...
Only the 5-year survival window (event censoring only, time unchanged).

Output: paper/KM/merlin/
"""
...
# e.g. "ConcatFC+ODST_WSI+CT_multimodal_..." → ("ConcatFC", "ODST")
...
def fix_score_direction(scores: np.ndarray, time: np.ndarray, event: np.ndarray) -> np.ndarray:
    """Flip scores if high score correlates with longer survival (wrong direction)."""
...
def make_km_figure(df: pd.DataFrame, cutoff: float, title: str = "") -> tuple[go.Figure, float, int, int]:
    df = df.copy()
    ...
    for g, label, color in [(1, "High Risk", COLORS["high"]),
        ...
        if not censored.empty:
            ...
            fig.add_trace(go.Scatter(
                x=censored["time"].tolist(), y=cens_probs,
...
def main():
    ...
    for mod_dir in modality_dirs:
        mod_label = mod_key_to_label(mod_dir.name)

        # Only directories that contain Concat* experiments
        exp_dirs = sorted([
        ...
        for exp_dir in exp_dirs:
            ...
            if csv_path is None:
                print(f"  [SKIP] {arch_label} {freeze_lbl}: no aggregate_5fold_test.csv")
                continue
            ...
            if len(common_ids) < 20:
                print(f"  [SKIP] {arch_label} {freeze_lbl}: only {len(common_ids)} common patients")
                continue
            ...
            if adaptive_cutoff is None:
                print(f"  [WARN] {arch_label} {freeze_lbl}: no valid adaptive cutoff, using median")
                adaptive_cutoff = median_cutoff

            safe_name = re.sub(r"[^\w\-]", "_", f"{mod_label}_{arch_label}_{freeze_lbl}")

            # Adaptive plot
            fig_a, p_a, _, _ = make_km_figure(df, adaptive_cutoff, title=f"<b>{mod_label}</b>")
            ...
            # Median plot
            fig_m, p_m, _, _ = make_km_figure(df, median_cutoff, title=f"<b>{mod_label}</b>")
    ...
    print(f"\nDone. {total_saved} plots saved to {OUT_DIR}/")
